...
function startSilentUpdateInstaller(installerPath, targetVersion) {
  ...
  const userStartMenuShortcut = path.join(
  ...
  const allUsersStartMenuShortcut = path.join(
  ...
  const lines = [
    ...
    `set "TEMPEST_START_MENU_USER=${userStartMenuShortcut.replace(/%/g, "%%").replace(/"/g, "")}"`,
    `set "TEMPEST_START_MENU_ALL=${allUsersStartMenuShortcut.replace(/%/g, "%%").replace(/"/g, "")}"`,
    ...
    "goto cleanup",
    ":launch_app",
    `> "%TEMPEST_UPDATE_STATE%" echo {"status":"running","stage":"relaunching","targetVersion":"%TEMPEST_TARGET_VERSION%","installedVersion":"!installed_version!","installerExitCode":!install_exit!,"launcherPath":"!verified_launcher!","logPath":"%TEMPEST_UPDATE_LOG%"}`,
    "set app_relaunched=",
    "set \"TEMPEST_VERIFIED_LAUNCHER=!verified_launcher!\"",
    "call :launch_with_start",
    "if defined app_relaunched goto launch_success",
    "call :launch_with_powershell",
    "if defined app_relaunched goto launch_success",
    "call :launch_with_explorer",
    "if defined app_relaunched goto launch_success",
    "call :launch_with_shortcut \"%TEMPEST_START_MENU_USER%\"",
    "if defined app_relaunched goto launch_success",
    "call :launch_with_shortcut \"%TEMPEST_START_MENU_ALL%\"",
    "if defined app_relaunched goto launch_success",
    `> "%TEMPEST_UPDATE_STATE%" echo {"status":"failed","stage":"relaunch-failed","targetVersion":"%TEMPEST_TARGET_VERSION%","installedVersion":"!installed_version!","installerExitCode":!install_exit!,"launcherPath":"!verified_launcher!","logPath":"%TEMPEST_UPDATE_LOG%"}`,
    `>> "%TEMPEST_UPDATE_LOG%" echo [%date% %time%] Update installed, but Tempest Light did not relaunch.`,
    "goto cleanup",
    ":launch_success",
    `> "%TEMPEST_UPDATE_STATE%" echo {"status":"success","stage":"relaunch-confirmed","targetVersion":"%TEMPEST_TARGET_VERSION%","installedVersion":"!installed_version!","installerExitCode":!install_exit!,"launcherPath":"!verified_launcher!","logPath":"%TEMPEST_UPDATE_LOG%"}`,
    `>> "%TEMPEST_UPDATE_LOG%" echo [%date% %time%] Tempest Light relaunched successfully.`,
    "del \"%TEMPEST_INSTALLER%\" >nul 2>nul",
    "goto cleanup",
    ":launch_with_start",
    `>> "%TEMPEST_UPDATE_LOG%" echo [%date% %time%] Relaunch attempt: cmd start !verified_launcher!.`,
    "for %%F in (\"!verified_launcher!\") do set \"verified_launcher_dir=%%~dpF\"",
    "start \"\" /d \"!verified_launcher_dir!\" \"!verified_launcher!\" --updated",
    "timeout /t 5 /nobreak >nul",
    "call :confirm_app_started",
    "exit /b 0",
    ":launch_with_powershell",
    `>> "%TEMPEST_UPDATE_LOG%" echo [%date% %time%] Relaunch attempt: PowerShell Start-Process.`,
    "powershell.exe -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command \"try { Start-Process -FilePath $env:TEMPEST_VERIFIED_LAUNCHER -ArgumentList '--updated' -WorkingDirectory (Split-Path -Parent $env:TEMPEST_VERIFIED_LAUNCHER) -WindowStyle Normal } catch { exit 1 }\" >nul 2>nul",
    "timeout /t 5 /nobreak >nul",
    "call :confirm_app_started",
    "exit /b 0",
    ":launch_with_explorer",
    `>> "%TEMPEST_UPDATE_LOG%" echo [%date% %time%] Relaunch attempt: explorer.exe !verified_launcher!.`,
    "explorer.exe \"!verified_launcher!\" >nul 2>nul",
    "timeout /t 5 /nobreak >nul",
    "call :confirm_app_started",
    "exit /b 0",
    ":launch_with_shortcut",
    "set candidate_shortcut=%~1",
    "if not exist \"%candidate_shortcut%\" exit /b 0",
    `>> "%TEMPEST_UPDATE_LOG%" echo [%date% %time%] Relaunch attempt: Start Menu shortcut %candidate_shortcut%.`,
    "start \"\" \"%candidate_shortcut%\"",
    "timeout /t 5 /nobreak >nul",
    "call :confirm_app_started",
    "exit /b 0",
    ":confirm_app_started",
    "tasklist /fi \"imagename eq Tempest Light.exe\" 2>nul | find /i \"Tempest Light.exe\" >nul",
    "if not errorlevel 1 set app_relaunched=1",
    "exit /b 0",
    ":check_candidate",
    "set candidate=%~1",
